...
int KeyExpansion(aeskey_t key, unsigned char w[]){
  ...
  //sizeof(unsigned char) is always 1 (unnecessary statement execution)
  //The for loop is being run at Nk=4,6,8 only. They are the only possible values. Furthermore, Nk=8 only in the 256-bit AES.
  //You can have one for loop for Nk=4 and 6, then another for Nk=8. Then run either one depending on Nk.
  //This results in faster for loops for all instances Nk=4,6,8 with fewer calculations in each.
  //Also, with Nk=8, i % Nk only happens for 12,20,28,36,44,52, so we can enumerate those.
  memcpy(w, key->key, 4*Nk);
  ...
  if(Nk == 4 || Nk == 6){
      ...
      for (i=8; i<60; i++){
        ...
        if (i==12 || i==20 || i==28 || i==36 || i==44 || i==52) {
          temp[0]= sbox[temp[0]];
          temp[1]= sbox[temp[1]];
          temp[2]= sbox[temp[2]];
          temp[3]= sbox[temp[3]];
...
//  AVOID FOR LOOPS, here we can enumerate the 16 elements of cipher mapped by sbox while saving time (saves about ~.10-.20 seconds on encryption)
...
//   NO NEED TO USE TEMP, STORING DATA IS TIME-EXPENSIVE, 
//   The new order is fixed so my solution is to simply memcpy the reordered array into the original cipher array
  memcpy(cipher, (unsigned char[]){cipher[0],cipher[5],cipher[10],cipher[15],cipher[4],cipher[9],
                                   cipher[14],cipher[3],cipher[8],cipher[13],cipher[2],cipher[7],
                                   cipher[12],cipher[1],cipher[6],cipher[11]},16);
...
//  KEEP WORKING WITH a[4] AND b[4], TRY TO AVOID UNNECESSARY FOR LOOPS (ESPECIALLY NESTED FOR LOOPS),
// My solution was to unload the inner for loop and enumerate b[0] to b[3] while keeping the same bit calculations needed for the function.
// Then unload the outer for loop and populate a exactly 4 times with 4 bytes each time.
// Then you can update cipher at the end without populating b with a nested for loop every time.
    ...
    cipher[0] = b[0] ^ a[3] ^ a[2] ^ b[1] ^ a[1];
    cipher[1] = b[1] ^ a[0] ^ a[3] ^ b[2] ^ a[2];
    cipher[2] = b[2] ^ a[1] ^ a[0] ^ b[3] ^ a[3];
    cipher[3] = b[3] ^ a[2] ^ a[1] ^ b[0] ^ a[0];
    ...
    cipher[4] = b[0] ^ a[3] ^ a[2] ^ b[1] ^ a[1];
    cipher[5] = b[1] ^ a[0] ^ a[3] ^ b[2] ^ a[2];
    cipher[6] = b[2] ^ a[1] ^ a[0] ^ b[3] ^ a[3];
    cipher[7] = b[3] ^ a[2] ^ a[1] ^ b[0] ^ a[0];
    ...
    cipher[8] = b[0] ^ a[3] ^ a[2] ^ b[1] ^ a[1];
    cipher[9] = b[1] ^ a[0] ^ a[3] ^ b[2] ^ a[2];
    cipher[10] = b[2] ^ a[1] ^ a[0] ^ b[3] ^ a[3];
    cipher[11] = b[3] ^ a[2] ^ a[1] ^ b[0] ^ a[0];
    ...
    cipher[12] = b[0] ^ a[3] ^ a[2] ^ b[1] ^ a[1];
    cipher[13] = b[1] ^ a[0] ^ a[3] ^ b[2] ^ a[2];
    cipher[14] = b[2] ^ a[1] ^ a[0] ^ b[3] ^ a[3];
    cipher[15] = b[3] ^ a[2] ^ a[1] ^ b[0] ^ a[0];
